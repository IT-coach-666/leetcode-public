...
解法1: 递归求解;
    ...
    def _ks(self, i, c):
        ...
        """
        # jy: 初始条件: 如果容量为 0, 或者已经没有待装物品了(即 i 为 0), 则最大价值为 0;
        if i == 0 or c == 0:
            result = 0
        ...
        # jy: 现有容量 c 可以装下第 i 个物品( c >= self.ws[i]), 则可以选择装下第 i 个物品,
        #     或不装第 i 个物品; (注意, 并不是选择装才能最大化价值, 比如但第 i 个物品很占容
        #     量, 但价值又极低时, 装该物品只会占用容量, 对价值增加没多大帮助, 此时不装该物
        #     品, 留足容量装其它物品就显得更有利于最大化价值)
        else:
            # jy: 如果选择不装第 i 个物品, 则从第 1 个物品到第 i-1 个物品选择性装入, 此时背
            #     包容量仍为 c;
            tmp1 = self._ks(i-1, c)
            # jy: 如果选择装入第 i 个物品, 则能获取到第 i 个物品的价值 self.vs[i], 同时背包
            #     的容量减小 self.ws[i], 此时可以继续从第 1 个物品到第 i-1 个物品中选择性装
            #     入, 此时的容量为 c - self.ws[i]
            tmp2 = self._ks(i-1, c - self.ws[i]) + self.vs[i]
            # jy: 在装与不装两种方案之间, 选择价值更大的方案
...
解法3: 动态规划
    ...
    def knapsack_01_v3(self, knapsack_capacity):
        ...
        #     初始化均为 0, 表示容量为 0 时不能装入任何物品, 收益为 0; dp 的长度为 knapsack_capacity + 1,
        #     确保容量为 knapsack_capacity 的最大收益直接使用 dp[knapsack_capacity] 就可表示;
        ...
        for i in range(1, len(self.vs) + 1):
            # jy: 注意, 背包容量从后往前(从最大容量往 0)遍历, 因为每个物品只能选择一次, 且 dp[j] 的
            #     计算依赖于上一轮外层 for 循环的 dp[j - self.ws[i]], 从后往前(即从大到小)遍历能确保
            #     计算 dp[j] 时使用的 dp[j - self.ws[i]] 是上一轮的结果(当前轮的该值更新需要等到 dp[j]
            #     更新完, j 逐渐变小后才会被更新为当前轮的更新结果)
            for j in range(knapsack_capacity, 0, -1):
                # jy: 确保 j - self.ws[i] >= 0 时, dp[j - self.ws[i]] 才有含义; 可以在以上内层 for 循环
                #     中直接写成 ``for j in range(knapsack_capacity, self.ws[i] - 1, -1)``, 确保每一个 j
                #     都是大于或等于 self.ws[i] 的, 因此可以省略掉 以下的 if 判断;
                if j >= self.ws[i]:
...
[0, 0, 2, 4, 4, 7, 7, 9, 11, 11, 13]
            ...
            """
            # jy: 如果 i 表示的是某一类物品, 且同一类物品在背包中可以存放多个, 则 j 的遍历需要从小到大,
            #     即当前轮次的 dp[j] 的计算依赖于 dp[j - self.ws[i]], 而 dp[j - self.ws[i]] 应先提前算
            #     出; 
            for j in range(self.ws[i], knapsack_capacity + 1):
                dp[j] = max(dp[j], dp[j - self.ws[i]] + self.vs[i])
            # jy: 每一轮对应的 dp 结果如下:
            print(dp)
            '''

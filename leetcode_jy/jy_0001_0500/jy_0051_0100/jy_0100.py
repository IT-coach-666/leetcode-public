...
Example 1:   https://www.yuque.com/frederick/dtwi9g/owoqhe
...
解法1: 递归求解
    ...
    """
解法2: 使用一个队列保存两个树对应的节点, 依次出队, 判断两节点是否相等
    """
    def isSameTree_v2(self, p: TreeNode, q: TreeNode) -> bool:
        # jy: 将两树的当前节点组成一个元组放入队列中, 后续队列不空则不断出队,
        #     并同时将出队的节点的左或右节点入队(入队时同时左或右, 保持一致)
        queue = deque([(p, q)])
        while queue:
            a, b = queue.popleft()
            # jy: 如果树节点均存在, 则判断节点值是否相等, 并对后续子节点入队;
            if a and b:
                if a.val != b.val:
                    return False
                queue.append((a.left, b.left))
                queue.append((a.right, b.right))
            # jy: 如果树节点均不存在, 则跳过, 继续从队列中出队(此时的队列不一定已空)
            elif not a and not b:
                continue
            # jy: 如果只有一个树节点存在(即另一个树节点为 None), 则直接返回 False;
            else:
                return False
